    ...
    !AxiECAMB: axion presence test, background solver driver, and Romberg helpers
    !that split integrals at the dtauda kink at the KG->EFA switch a_osc
    procedure :: HasAxion => CAMBdata_HasAxion
    procedure, private :: AxionSolve => CAMBdata_AxionSolve
    procedure, private :: AxKinkRomberg_a
    procedure, private :: AxKinkRomberg_z
...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
        ...
        h2 = (this%CP%H0/100)**2
        !AxiECAMB: resolve the axion abundance and the effective CDM density
        !(inidriver_axion.F90:272-312 logic; m/H0 >= 10 is the DM/DE discriminator)
        if (allocated(this%CP%Axion)) then
            if (this%CP%Axion%active) then
                associate (Ax => this%CP%Axion)
                    Ax%H0_eV = h_P*(this%CP%H0/(c/1.0d3))/(MPC_in_sec*2._dl*const_pi*eV)
                    Ax%m_ovH0 = Ax%m_ax/Ax%H0_eV
                    Ax%is_de_like = Ax%m_ovH0 < 10._dl
                    if (Ax%use_axfrac) then
                        if (.not. Ax%is_de_like) then
                            !DM case: axfrac is the axion fraction of the dark matter
                            Ax%omaxh2_eff = Ax%axfrac*Ax%omdah2
                            this%CP%omch2 = (1._dl - Ax%axfrac)*Ax%omdah2
                        else
                            !DE case: all of omdah2 goes to CDM, axfrac is the ULA
                            !fraction of the dark energy budget
                            if (FeedbackLevel > 0) &
                                write(*,*) 'Note: m/H0 < 10, axfrac is ULA fraction in dark energy'
                            this%CP%omch2 = Ax%omdah2
                            Ax%omaxh2_eff = Ax%axfrac*(1._dl - (this%CP%ombh2 + this%CP%omch2 + &
                                this%CP%omnuh2)/h2 - this%CP%omk - &
                                (this%grhornomass + this%grhog)/this%grhocrit)*h2
                        end if
                    else
                        Ax%omaxh2_eff = Ax%omaxh2
                    end if
                end associate
            else
                this%CP%Axion%omaxh2_eff = 0._dl
            end if
        end if
        ...
        !AxiECAMB: the axion is part of the budget, so the dark energy closes with it
        if (this%HasAxion()) this%Omega_de = this%Omega_de - this%CP%Axion%omaxh2_eff/h2
        ...
        if (this%HasAxion()) then
            !DM-like axions count as matter in equality (AxiECAMB modules.f90:2917)
            if (.not. this%CP%Axion%is_de_like) &
                this%z_eq = (this%grhob+this%grhoc+this%grhocrit*this%CP%Axion%omaxh2_eff/h2)/ &
        ...
        !AxiECAMB: solve the axion KG background after neutrinos but before the dark
        !energy (the solver treats DE as Lambda; quintessence-type DE is rejected in
        !Validate). After this, grho_no_de/dtauda include the axion automatically.
        if (this%HasAxion()) then
            call this%AxionSolve()
            !LCDM C_l template interpolation biases axion spectra (AxiECAMB removed
            !templated interpolation in InterpolateCls)
            this%CP%use_cl_spline_template = .false.
        end if
        if (global_error_flag/=0) then
            if (present(error)) error = global_error_flag
            return
        ...
        if (global_error_flag==0 .and. this%HasAxion()) then
            !conformal time of the KG->EFA switch (AxiECAMB init_background, tol 1e-8)
            associate (Ax => this%CP%Axion)
                if (Ax%has_switch .and. Ax%a_osc <= 1._dl) then
                    Ax%tau_osc = this%DeltaTime(0._dl, Ax%a_osc, in_tol=1.0d-8)
                    if (Ax%tau_osc > this%tau0) then
                        write(*,*) 'a_osc <= 1 and tau_osc > tau0, clamping tau_osc to tau0'
                        Ax%tau_osc = min(Ax%tau_osc, this%tau0)
                    end if
                else
                    Ax%tau_osc = this%tau0 + 1._dl  !no switch before the present day
                end if
            end associate
        end if
        if (global_error_flag==0) then
    ...
    !AxiECAMB: was Integrate_Romberg(this, dtauda, a1, a2, atol); the helper splits the
    !integral at a_osc where dtauda has a kink (no-op without an active axion switch)
    CAMBdata_DeltaTime = this%AxKinkRomberg_a(dtauda,a1,a2,atol)

    end function CAMBdata_DeltaTime

    logical function CAMBdata_HasAxion(this)
    !whether an active AxiECAMB ultralight-axion component is present
    class(CAMBdata) :: this

    CAMBdata_HasAxion = allocated(this%CP%Axion)
    if (CAMBdata_HasAxion) CAMBdata_HasAxion = this%CP%Axion%active
    end function CAMBdata_HasAxion

    function AxKinkRomberg_a(this, fin, a1, a2, atol, maxit, minsteps) result(int)
    !Romberg integral over scale factor, split at the axion KG->EFA switch a_osc
    !where dtauda has a kink (AxiECAMB modules.f90 DeltaTime treatment, applied
    !uniformly to all background integrals)
    class(CAMBdata) :: this
    real(dl), external :: fin
    real(dl), intent(in) :: a1, a2, atol
    ...
    split = .false.
    if (this%HasAxion()) then
        if (this%CP%Axion%has_switch) then
            aosc = this%CP%Axion%a_osc
            split = a1 < aosc .and. a2 >= aosc
        end if
    end if
    if (split) then
        int = Integrate_Romberg(this, fin, a1, aosc*(1._dl - max(atol/100.0_dl, 1.d-15)), &
            atol, maxit, minsteps) + Integrate_Romberg(this, fin, aosc, a2, atol, maxit, minsteps)
    else
        int = Integrate_Romberg(this, fin, a1, a2, atol, maxit, minsteps)
    end if
    end function AxKinkRomberg_a

    function AxKinkRomberg_z(this, fin, z1, z2, atol, maxit, minsteps) result(int)
    !Same as AxKinkRomberg_a for integrals over redshift (kink at z_osc = 1/a_osc - 1)
    class(CAMBdata) :: this
    real(dl), external :: fin
    real(dl), intent(in) :: z1, z2, atol
    integer, intent(in), optional :: maxit, minsteps
    real(dl) int, zosc
    logical split
    ...
    split = .false.
    if (this%HasAxion()) then
        if (this%CP%Axion%has_switch .and. this%CP%Axion%a_osc < 1._dl) then
            zosc = 1._dl/this%CP%Axion%a_osc - 1._dl
            split = z1 < zosc .and. z2 > zosc
        end if
    end if
    if (split) then
        int = Integrate_Romberg(this, fin, z1, zosc*(1._dl - max(atol/100.0_dl, 1.d-15)), &
            atol, maxit, minsteps) + Integrate_Romberg(this, fin, zosc, z2, atol, maxit, minsteps)
    else
        int = Integrate_Romberg(this, fin, z1, z2, atol, maxit, minsteps)
    end if
    end function AxKinkRomberg_z

    subroutine CAMBdata_AxionSolve(this)
    !Solve the axion KG background (with the internal dfac retuning) using background
    !pieces from this state so the solver's H(a) matches grho_no_de exactly
    class(CAMBdata), target :: this
    type(TAxionBgInputs) :: BG
    integer badflag, nu_i, neig
    real(dl) h2

    h2 = (this%CP%H0/100)**2
    BG%hnot = this%CP%H0/100._dl
    BG%omegah2_b = this%CP%ombh2
    BG%omegah2_dm = this%CP%omch2
    BG%omegah2_nu = this%CP%omnuh2
    BG%omegah2_lambda = this%Omega_de*h2   !DE treated as Lambda in the solver
    BG%omk = this%CP%omk
    BG%omegah2_gamma = this%grhog/this%grhocrit*h2
    BG%omegah2_rad = (this%grhog + this%grhornomass)/this%grhocrit*h2
    BG%nu_massless_degeneracy = this%grhornomass/this%grhor
    BG%sum_nu_mass_degeneracies = sum(this%CP%Nu_mass_degeneracies(1:this%CP%Nu_mass_eigenstates))
    !mirror the grho_no_de massive-neutrino condition exactly
    if (this%CP%Num_Nu_massive /= 0) then
        neig = this%CP%Nu_mass_eigenstates
    else
        neig = 0
    end if
    BG%Nu_mass_eigenstates = neig
    allocate(BG%nu_masses(max(neig,1)), BG%lhsqcont_massive(max(neig,1)))
    BG%nu_masses = 0._dl
    BG%lhsqcont_massive = 0._dl
    do nu_i = 1, neig
        BG%nu_masses(nu_i) = this%nu_masses(nu_i)
        BG%lhsqcont_massive(nu_i) = this%grhormass(nu_i)/this%grhocrit*h2
    end do
    !ScalarPowerAmp only feeds the isocurvature amplitude bookkeeping, which is
    !force-disabled in this release (AxiECAMB v1.0 parity)
    BG%ScalarPowerAmp = 0._dl
    BG%FeedbackLevel = FeedbackLevel

    call this%CP%Axion%Solve(BG, badflag)
    if (badflag /= 0 .and. global_error_flag == 0) &
        call GlobalError('TAxionModel: axion background solve failed', error_unsupported_params)
    end subroutine CAMBdata_AxionSolve
    ...
    !AxiECAMB: integral split at the a_osc kink (no-op without an active axion switch)
    CAMBdata_DeltaPhysicalTimeGyr = this%AxKinkRomberg_a(dtda,a1,a2,atol)*Mpc/c/Gyr
    ...
    !AxiECAMB: integral split at the a_osc kink (no-op without an active axion switch)
    CAMBdata_sound_horizon = this%AxKinkRomberg_a(dsound_da_exact,1d-9,1/(z+1),1e-6_dl)
    ...
    !AxiECAMB counts the axion in the dark matter for the z_star fit (modules.f90:741)
    if (this%HasAxion()) omdmh2 = omdmh2 + this%CP%Axion%omaxh2_eff
    ...
    astar = 1/(1+zstar)
    atol = 1e-6
    rs = this%AxKinkRomberg_a(dsound_da_approx,1d-8,astar,atol) !AxiECAMB: split at a_osc
    ...
    noreion_optdepth = this%AxKinkRomberg_z(noreion_doptdepth_dz, 0.d0, z, 1d-5, 20, 100) !AxiECAMB: split at z_osc
    ...
    dragoptdepth =  this%AxKinkRomberg_z(ddragoptdepth_dz, 0.d0, z, 1d-5, 20, 100) !AxiECAMB: split at z_osc
    ...
    !AxiECAMB ultralight axion: exact KG density before the switch, EFA after
    if (this%HasAxion()) grhoa2 = grhoa2 + this%CP%Axion%GrhoAx(a)
    ...
    !AxiECAMB: split at z_osc (light axions can switch within the reionization range)
    GetReionizationOptDepth = this%AxKinkRomberg_z(reion_doptdepth_dz,0.d0,zstart,&
    ...
    if (CP%WantDerivedParameters) then
        associate(ThermoDerivedParams => State%ThermoDerivedParams)
            ...
            ThermoDerivedParams( derived_kD ) =  &  !AxiECAMB: damping integral split at a_osc
                sqrt(1.d0/(State%AxKinkRomberg_a(ddamping_da, 1d-8, 1/(this%z_star+1), 1d-6)/6))
            !$OMP SECTION
    ...
    if (State%HasAxion()) then
        !Cache opacity and exp(-kappa) at the KG->EFA switch, used by the boundary
        !term in the temperature source integral (AxiECAMB ThermoSplineOut analog)
        associate (Ax => State%CP%Axion)
            if (Ax%has_switch .and. Ax%tau_osc < State%tau0 .and. Ax%tau_osc > this%tauminn) then
                block
                    real(dl) a_x, opac_x, dopac_x, ddopac_x, vis_x, dvis_x, ddvis_x, expmmu_x, lenswin_x
                    call this%IonizationFunctionsAtTime(Ax%tau_osc, a_x, opac_x, dopac_x, ddopac_x, &
                        vis_x, dvis_x, ddvis_x, expmmu_x, lenswin_x)
                    Ax%opac_tauosc = opac_x
                    Ax%expmmu_tauosc = expmmu_x
                end block
            else
                !switch before the start of the thermal history table: e^-kappa ~ 0
                !so the boundary term is exponentially suppressed
                Ax%opac_tauosc = 0._dl
                Ax%expmmu_tauosc = 0._dl
            end if
        end associate
    end if
    ...
    !AxiECAMB: refine the time sampling around the KG->EFA switch tau_osc so the
    !sources resolve the matching (modules.f90:3044-3073: ultra-fine window of
    !+-6.5 steps with dtauosc = tau_osc/int(6000*dfac), plus a 1000x coarser
    !refinement from taurend to tau_osc)
    if (State%HasAxion()) then
        associate (Ax => State%CP%Axion)
            if (Ax%has_switch .and. Ax%a_osc <= 1._dl) then
                if (Ax%tau_osc > State%taurst) then
                    delta = Ax%tau_osc/int(6000._dl*Ax%dfac_used)
                    call TimeSteps%Add_delta(max(Ax%tau_osc - 6.5_dl*delta, State%taurst), &
                        min(Ax%tau_osc + 6.5_dl*delta, State%tau0), delta)
                end if
                if (Ax%tau_osc > State%taurend) then
                    delta = Ax%tau_osc/int(6000._dl*Ax%dfac_used)*1000._dl
                    call TimeSteps%Add_delta(max(6.5_dl*delta, State%taurend), &
                        min(Ax%tau_osc + 6.5_dl*delta, State%tau0), delta)
                end if
            end if
        end associate
    ...
    ! total perturbations with and without neutrinos, with neutrinos+dark energy in the numerator
        ...
        Transfer_axion = 14 !AxiECAMB ultralight-axion density perturbation
    !Sources
    !Alternatively for 21cm
    integer, parameter :: Transfer_monopole=4, Transfer_vnewt=5, Transfer_Tmat = 6

    integer, parameter :: Transfer_max = Transfer_axion !AxiECAMB: was Transfer_vel_baryon_cdm
    character(LEN=name_tag_len) :: Transfer_name_tags(Transfer_max-1) = &
        ...
        'axion   '] !AxiECAMB: axion column appended
