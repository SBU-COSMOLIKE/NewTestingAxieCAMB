    ...
    type IntegrationVars
        ...
        !AxiECAMB: switch boundary amplitudes interpolated to this k (temperature)
        real(dl) metricdeltas_q(2)
    ...
    !AxiECAMB: per-evolved-k amplitudes of the delta-function boundary term in the
    !temperature source at the KG->EFA switch (only allocated for scalar passes with
    !an active axion switch before today); second index 1:2 = (J_l, dJ_l/dtau) pieces
    real(dl), dimension(:,:), allocatable :: deltaBCSrc, dd_deltaBCSrc
    ...
    real(dl) tauosc, om, rhomass, grhonu, taueq !AxiECAMB: axion start-time constraints
    ...
    !AxiECAMB: start well before the axion oscillation and matter-radiation equality
    !(AxiECAMB cmbmain.f90 GetTauStart, 770-830)
    if (State%HasAxion()) then
        associate (Ax => CP%Axion)
            ...
            om = (State%grhob + State%grhoc)/sqrt(3.0d0*(State%grhog + grhonu))
            taueq = (-1.0d0 + dsqrt(1.0d0 + Ax%aeq*0.01d0/State%adotrad))/om/2.0d0
            if (Ax%has_switch) then
                tauosc = (Ax%a_osc*0.01d0/State%adotrad)/om
                taustart = min(taustart, tauosc, taueq, 0.3_dl*Ax%tau_osc)
            else
                taustart = min(taustart, taueq)
            end if
            if (taustart <= State%DeltaTime(0._dl, Ax%a_table_min, 1.0d-10)) then
                write(*,*) 'WARNING: perturbation taustart is before background taustart, ', &
                    ...
                    taustart, State%DeltaTime(0._dl, Ax%a_table_min, 1.0d-10)
            end if
        end associate
    end if
    ...
    !AxiECAMB: boundary-term amplitudes only exist for the scalar pass when the
    !KG->EFA switch happens before today
    if (allocated(deltaBCSrc)) deallocate(deltaBCSrc, dd_deltaBCSrc)
    if (CP%WantScalars .and. State%HasAxion()) then
        if (CP%Axion%has_switch .and. CP%Axion%tau_osc < State%tau0) then
            allocate(deltaBCSrc(ThisSources%Evolve_q%npoints,2), source=0._dl)
            allocate(dd_deltaBCSrc(ThisSources%Evolve_q%npoints,2), source=0._dl)
        end if
    end if
...
101         if (CP%WantTransfer.and.itf <= State%num_transfer_redshifts) then
    ...
    !AxiECAMB: record the switch boundary amplitudes for this k (temperature source)
    if (allocated(deltaBCSrc)) then
        deltaBCSrc(EV%q_ix,:) = EV%metric_delta
    end if
    ...
    !AxiECAMB: interpolate the switch boundary amplitudes across k as well
    if (allocated(deltaBCSrc)) then
        do i = 1, 2
            call spline_def(ThisSources%Evolve_q%points, deltaBCSrc(:,i), &
                ThisSources%Evolve_q%npoints, dd_deltaBCSrc(:,i))
        end do
    end if
    ...
    !AxiECAMB: interpolate the switch boundary amplitudes to this k
    if (allocated(deltaBCSrc)) then
        do i = 1, 2
            IV%metricdeltas_q(i) = a0*deltaBCSrc(klo,i) + b0*deltaBCSrc(khi,i) + &
                (a03*dd_deltaBCSrc(klo,i) + b03*dd_deltaBCSrc(khi,i))*ho2o6
        end do
    end if
    ...
    real(dl) qmax_int
    real(dl) xf_osc, Jl_osc, dotJl_osc !AxiECAMB switch boundary term
    ...
    do j=1,max_bessels_l_index
        ...
        !AxiECAMB: delta-function boundary term in the temperature source at the
        !KG->EFA switch (AxiECAMB cmbmain.f90:1863-1927). Integrating the source
        !discontinuity against j_l(k(tau0-tau)) gives a J_l piece with amplitude
        !metricdeltas_q(1) and a dJ_l/dtau piece with amplitude metricdeltas_q(2);
        !the 11/10 opacity compensation is hard-coded upstream (RL 090424). Point
        !contribution: NOT multiplied by the time-step measure.
        if (allocated(deltaBCSrc)) then
            associate (Ax => CP%Axion)
                xf_osc = abs(IV%q*(State%tau0 - Ax%tau_osc))
                if (xf_osc > BessRanges%points(1) .and. xf_osc < BessRanges%points(BessRanges%npoints)) then
                    bes_ix = BessRanges%IndexOf(xf_osc)
                    dx = BessRanges%points(bes_ix+1) - BessRanges%points(bes_ix)
                    a2 = (BessRanges%points(bes_ix+1) - xf_osc)/dx
                    base = bessel_horner(1,bes_ix,j)
                    coeff1 = bessel_horner(2,bes_ix,j)
                    coeff2 = bessel_horner(3,bes_ix,j)
                    coeff3 = bessel_horner(4,bes_ix,j)
                    Jl_osc = base + a2*(coeff1 + a2*(coeff2 + a2*coeff3))
                    !x = q(tau0-tau) so dJ_l/dtau = q P'(a2)/dx with P the Horner spline
                    dotJl_osc = IV%q*(coeff1 + a2*(2*coeff2 + 3*coeff3*a2))/dx
                    sums(1) = sums(1) + Ax%expmmu_tauosc*Jl_osc*(IV%metricdeltas_q(1) &
                        - Ax%opac_tauosc*IV%metricdeltas_q(2)*11._dl/10._dl) &
                        + Ax%expmmu_tauosc*dotJl_osc*IV%metricdeltas_q(2)
                end if
            end associate
        end if
    ...
    real(dl) chi_osc, sh_osc, ujl_osc, dotujl_osc, t_lo, t_hi !AxiECAMB boundary term
    ...
    !AxiECAMB: switch boundary term, curved case (AxiECAMB cmbmain.f90:2315-2329).
    !Point contribution added after the measure; fires only in the sub-range that
    !straddles tau_osc (half-open test prevents double counting at range edges).
    if (allocated(deltaBCSrc)) then
        associate (Ax => CP%Axion)
            t_lo = min(State%TimeSteps%points(nstart), State%TimeSteps%points(nend))
            t_hi = max(State%TimeSteps%points(nstart), State%TimeSteps%points(nend))
            if (Ax%tau_osc < State%tau0 .and. t_lo <= Ax%tau_osc .and. Ax%tau_osc < t_hi) then
                chi_osc = (State%tau0 - Ax%tau_osc)/State%curvature_radius
                call USpherBesselWithDeriv(State%closed, CP, chi_osc, l, nu, ujl_osc, dotujl_osc)
                sh_osc = State%rofChi(chi_osc)
                ujl_osc = ujl_osc/sh_osc
                dotujl_osc = -(dotujl_osc - ujl_osc*State%cosfunc(chi_osc))/(sh_osc*State%curvature_radius)
                out(1) = out(1) + Ax%expmmu_tauosc*ujl_osc*(IV%metricdeltas_q(1) &
                    - Ax%opac_tauosc*IV%metricdeltas_q(2)*11._dl/10._dl) &
                    + Ax%expmmu_tauosc*dotujl_osc*IV%metricdeltas_q(2)
            end if
        end associate
    end if
    ...
    if (allocated(deltaBCSrc)) deallocate(deltaBCSrc, dd_deltaBCSrc) !AxiECAMB
