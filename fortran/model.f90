    ...
    type, extends (TCAMBParameters) :: CAMBparams
        ...
        !AxiECAMB ultralight-axion component (allocated with active=.false. by default)
    ...
    !AxiECAMB: validate the axion parameters and supported model combinations
    if (allocated(this%Axion)) then
        call this%Axion%Validate(OK)
        if (this%Axion%active) then
            select type (DE => this%DarkEnergy)
            class is (TDarkEnergyEqnOfState)
                !fluid/PPF dark energy is fine
            class default
                !the axion background solver treats dark energy as Lambda; scalar-field
                !DE models would need a coupled background solve - not supported
                OK = .false.
                write(*,*) 'TAxionModel: only fluid/ppf dark energy supported with axions'
            end select
            if (this%Do21cm .or. this%Evolve_delta_xe .or. this%Evolve_delta_Ts) then
                write(*,*) 'WARNING: 21cm/ionization perturbations untested with axions'
            end if
        end if
    end if
