    ...
    if (timeTh < H_frac*timeH) then
        ...
        if (Recomb%State%HasAxion()) then
            !AxiECAMB: dHdz including the axion (recfast_axion.F90:1058-1100), as a
            !numerical derivative of the exact H(z) from dtauda with the finite-
            !difference step directed away from the KG->EFA kink at a_osc
            block
                real(dl) a_here, a_step, Hz2, z2
                a_here = 1.d0/ainv
                if (Recomb%State%CP%Axion%has_switch .and. a_here < Recomb%State%CP%Axion%a_osc) then
                    a_step = a_here*(1.d0 - 1.d-3)
                else
                    a_step = a_here*(1.d0 + 1.d-3)
                end if
                z2 = 1.d0/a_step - 1.d0
                Hz2 = (1.d0/a_step**2)/dtauda(Recomb%State, a_step)/MPC_in_sec
                dHdz = (Hz2 - Hz)/(z2 - z)
            end block
        else
