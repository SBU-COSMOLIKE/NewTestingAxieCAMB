    ...
    use results
    use MassiveNu
    use AxionBackground !AxiECAMB: TAxionModel ultralight-axion component
    use DarkEnergyInterface
    use Transfer
    implicit none
    public
    ...
    type EvolutionVars
        ...
        !AxiECAMB ultralight axion
        integer axion_ix !Index of the two axion equations: (delta-phi/renorm_c, delta-phi-dot)
        !                 before the KG->EFA switch, (delta_ax, u_ax=(1+w)v) after
        logical oscillation_started !whether the KG->EFA switch has been performed for this k
        real(dl) metric_delta(2) !source jumps at the switch, exported to cmbmain as a
        !                         delta-function boundary term in the LOS integral
        real(dl) renorm_c !per-k rescaling keeping the stored delta-phi O(1)
    ...
    real(dl) tau_switch_oscillation !AxiECAMB axion KG->EFA switch

    noSwitch= State%tau0+1
    smallTime =  min(tau, 1/EV%k_buf)/100

    !AxiECAMB: the axion oscillation switch, checked first
    tau_switch_oscillation = noSwitch
    if (State%HasAxion()) then
        if (CP%Axion%has_switch .and. .not. EV%oscillation_started) &
            tau_switch_oscillation = CP%Axion%tau_osc
    end if
    ...
    !AxiECAMB: tau_switch_oscillation added (first) to the standard switch chain
    next_switch = min(tau_switch_oscillation, tau_switch_ktau, tau_switch_nu_massless, &
        EV%TightSwitchoffTime, tau_switch_nu_massive, &
    ...
    if (next_switch < tauend) then
        ...
        if (next_switch == tau_switch_oscillation) then
            !AxiECAMB: axion KG->EFA switch; integrate exactly to tau_osc, project the
            !field perturbations onto the effective fluid, restart dverk
            EVout%oscillation_started = .true.
            call SetupScalarArrayIndices(EVout)
            call CopyScalarVariableArray(y,yout, EV, EVout)
            call AxionSwitchKGtoEFA(EV, EVout, y, yout, tau)
            EV=EVout
            y=yout
            ind=1
    ...
    !AxiECAMB axion: two equations in both regimes (KG before the switch, EFA after)
    if (State%HasAxion()) then
        EV%axion_ix = neq + 1
        neq = neq + 2
        maxeq = maxeq + 2
    ...
    !AxiECAMB axion (the KG->EFA variable transformation at the switch is done
    !separately in AxionSwitchKGtoEFA after this plain copy)
    if (State%HasAxion()) then
        yout(EVout%axion_ix) = y(EV%axion_ix)
        yout(EVout%axion_ix+1) = y(EV%axion_ix+1)
    end if
    ...
    subroutine AxionSwitchKGtoEFA(EV, EVout, y, yout, tau)
    !AxiECAMB: one-time projection of the axion Klein-Gordon perturbations
    !(delta-phi, delta-phi-dot) onto the cycle-averaged effective-fluid variables
    !(delta_ax, u_ax) at tau_osc, including the eta shift / source-jump bookkeeping.
    !Ported from AxiECAMB equations_ppf.f90 CopyScalarVariableArray lines 1017-1146.
    !Nov13 -> modern state mapping: y(1)=a -> ThermoData; y(2) -> y(ix_etak);
    !yprime(3)=clxcdot -> yprime(ix_clxc); yprime(1)/a = adotoa per side -> computed
    !from the per-side background density below.
    use constants, only : c
    type(EvolutionVars) EV, EVout
    real(dl), intent(in) :: y(EV%nvar)
    real(dl), intent(inout) :: yout(EVout%nvar)
    real(dl), intent(in) :: tau
    real(dl) yprime(EV%nvar), yprimeout(EVout%nvar)
    real(dl) a, a2, k, k2, cs2x, opacity
    real(dl) v1_bg, v2_bg, w_ax, wcorr_coeff
    real(dl) tU, tV, tW, tdvarphi_c, tdvarphi_cp, tdvarphi_s, tdvarphi_sp
    real(dl) tdrho_ef, u_ax_ef, u_ax_efa, weight
    real(dl) H0_mpc, hnot, h2, m_mpc
    real(dl) grho_base, grhonu_t, grhov_t, w_dark_energy_t, rhonu
    real(dl) hub_kg, hub_efa, sigma_kg, sigma_efa
    integer nu_i

    k = EV%k_buf
    k2 = EV%k2_buf
    call EV%ThermoData%Values(tau, a, cs2x, opacity)
    a2 = a*a

    associate (Ax => CP%Axion)
        hnot = CP%H0/100._dl
        h2 = hnot**2
        H0_mpc = CP%H0/(c/1.0d3)
        m_mpc = Ax%m_ovH0*H0_mpc

        call Ax%FieldValsAta(a, v1_bg, v2_bg)
        wcorr_coeff = Ax%wcorr_coeff
        w_ax = ((wcorr_coeff/a2)**2.0d0)*Ax%wEFA_c

        !KG-side derivatives at tau_osc (for hLdot = 2 k z = -2 clxcdot and etakdot)
        yprime = 0
        call derivs(EV, EV%ScalEqsToPropagate, tau, y, yprime)

        !U, V, W coefficients of the 2x2 system for the perturbed WKB amplitudes
        tU = ((-Ax%tvarphi_c + Ax%tvarphi_sp)*2.0d0*yprime(ix_clxc) &
            - 2.0d0*k2*y(EV%axion_ix+1)/(a2*(Ax%m_ovH0**2.0d0)*H0_mpc))/(a*Ax%m_ovH0*H0_mpc) &
            + 6.0d0*Ax%ah_osc*y(EV%axion_ix)*EV%renorm_c/(a*Ax%m_ovH0*hnot)
        tV = (-(Ax%tvarphi_s + Ax%tvarphi_cp)*2.0d0*yprime(ix_clxc) &
            + 2.0d0*k2*y(EV%axion_ix)*EV%renorm_c/(a*Ax%m_ovH0*H0_mpc))/(a*Ax%m_ovH0*H0_mpc) &
            + 6.0d0*Ax%ah_osc*y(EV%axion_ix+1)/(a2*(Ax%m_ovH0**2.0d0)*hnot)
        tW = Ax%A_coeff**2.0d0 + 3.0d0*Ax%A_coeff*Ax%ah_osc/(a*Ax%m_ovH0*hnot) &
            + 2.0d0*k2/((a*Ax%m_ovH0*H0_mpc)**2.0d0) + 4.0d0

        !the perturbed cos/sin amplitudes
        tdvarphi_c = y(EV%axion_ix)*EV%renorm_c
        tdvarphi_cp = (-2.0d0*tU - (Ax%A_coeff + 3.0d0*Ax%ah_osc/(a*Ax%m_ovH0*hnot))*tV)/(2.0d0*tW)
        tdvarphi_s = y(EV%axion_ix+1)/(a*Ax%m_ovH0) - tdvarphi_cp
        tdvarphi_sp = (Ax%A_coeff*tU - (2.0d0 + k2/((a*Ax%m_ovH0*H0_mpc)**2.0d0))*tV)/(2.0d0*tW)

        !cycle-averaged effective-fluid delta-rho (normalized later)
        tdrho_ef = (Ax%m_ovH0**2.0d0)*(Ax%tvarphi_s*tdvarphi_cp - Ax%tvarphi_c*tdvarphi_sp &
            + Ax%tvarphi_cp*tdvarphi_cp + Ax%tvarphi_sp*tdvarphi_sp &
            + tdvarphi_s*(2.0d0*Ax%tvarphi_s + Ax%tvarphi_cp) &
            + tdvarphi_c*(2.0d0*Ax%tvarphi_c - Ax%tvarphi_sp))

        !normalized u_ax_ef = (1+w) theta_ax_ef/k
        u_ax_ef = k*Ax%m_ovH0*(tdvarphi_c*(Ax%tvarphi_s + Ax%tvarphi_cp) &
            + tdvarphi_s*(-Ax%tvarphi_c + Ax%tvarphi_sp))/(a*H0_mpc)
        u_ax_ef = u_ax_ef/(Ax%rhorefp_ovh2*h2)

        !sub/super-horizon weight for distributing the source jump
        weight = (k/(Ax%ahosc_ETA*H0_mpc/hnot))**2._dl/ &
            (3._dl + (k/(Ax%ahosc_ETA*H0_mpc/hnot))**2._dl)

        !rescale the flux so the velocity v (not u) is continuous across the switch
        u_ax_efa = u_ax_ef*(1._dl + w_ax)/(1._dl + (Ax%Prefp/(Ax%rhorefp_ovh2*h2)))

        yout(EVout%axion_ix) = tdrho_ef/(Ax%rhorefp_ovh2*h2)  !deltaax_ef
        !shift delta so the rest-frame density/sigma are continuous
        yout(EVout%axion_ix) = yout(EVout%axion_ix) + &
            (3._dl*Ax%ah_osc*H0_mpc/hnot)*(u_ax_ef - u_ax_efa)/k
        yout(EVout%axion_ix+1) = u_ax_efa  !u_ax, taken to be continuous (= u_ax_efa)

        !EFA-side derivatives with the transformed variables
        yprimeout = 0
        call derivs(EVout, EVout%ScalEqsToPropagate, tau, yout, yprimeout)

        !per-side conformal Hubble (the densities differ across the switch because the
        !EFA density is the cycle average, not the instantaneous KG value)
        grho_base = State%grhob/a + State%grhoc/a + State%grhornomass/a2 + State%grhog/a2
        if (CP%Num_Nu_massive /= 0) then
            grhonu_t = 0._dl
            do nu_i = 1, CP%Nu_mass_eigenstates
                call ThermalNuBack%rho(a*State%nu_masses(nu_i), rhonu)
                grhonu_t = grhonu_t + State%grhormass(nu_i)*rhonu/a2
            end do
            grho_base = grho_base + grhonu_t
        end if
        ...
        grho_base = grho_base + grhov_t
        hub_kg = sqrt((grho_base + grhom_ovh2*((v2_bg/a)**2.0d0 + (Ax%m_ovH0*v1_bg)**2.0d0)*a2 &
            + State%grhok)/3._dl)
        hub_efa = sqrt((grho_base + State%grhocrit*Ax%rhorefp_ovh2*((Ax%a_osc/a)**3.0d0)* &
            dexp((wcorr_coeff**2.0d0)*3.0d0*Ax%wEFA_c*(1.0d0/(a2**2.0d0) - &
            1.0d0/(Ax%a_osc**4.0d0))/4.0d0)*a2 + State%grhok)/3._dl)

        !source jumps: sigma = -clxcdot/k + 3*etakdot/k^2 (flat), curvature variant below
        if (State%flat) then
            sigma_kg = (-yprime(ix_clxc)/k + 3._dl*yprime(ix_etak)/k2)
            sigma_efa = (-yprimeout(ix_clxc)/k + 3._dl*yprimeout(ix_etak)/k2)
            EVout%metric_delta(1) = (sigma_kg*(2._dl*hub_kg)/k - y(ix_etak)/k) - &
                (sigma_efa*(2._dl*hub_efa)/k - yout(ix_etak)/k)
            EVout%metric_delta(2) = sigma_kg/k - sigma_efa/k
        else
            sigma_kg = (-yprime(ix_clxc)/k + 3._dl*(yprime(ix_etak) - &
                State%curv*(-yprime(ix_clxc)/k))/k2)
            sigma_efa = (-yprimeout(ix_clxc)/k + 3._dl*(yprimeout(ix_etak) - &
                State%curv*(-yprimeout(ix_clxc)/k))/k2)
            EVout%metric_delta(1) = (sigma_kg*(2._dl*hub_kg)/(k*EV%Kf(1)) - y(ix_etak)/(k*EV%Kf(1))) - &
                (sigma_efa*(2._dl*hub_efa)/(k*EVout%Kf(1)) - yout(ix_etak)/(k*EV%Kf(1)))
            EVout%metric_delta(2) = sigma_kg/(k*EV%Kf(1)) - sigma_efa/(k*EVout%Kf(1))
        end if

        !AxiECAMB: sub-horizon, absorb the jump into eta itself; super-horizon, export
        !it to the line-of-sight boundary term via metric_delta -> deltaBCSrc in cmbmain
        yout(ix_etak) = (EVout%metric_delta(2)*hub_efa)*(k*EV%Kf(1))*weight + y(ix_etak)
        EVout%metric_delta(1) = EVout%metric_delta(1) - weight*hub_efa*EVout%metric_delta(2)
        EVout%metric_delta(2) = EVout%metric_delta(2)*(1._dl - weight)
    end associate

    end subroutine AxionSwitchKGtoEFA !AxiECAMB
    ...
    if (CP%WantScalars) then
        ...
        if (EV%q < 0.05) then
            ...
            if (State%HasAxion()) then
                !AxiECAMB "WH smoother lmaxnr": more neutrino multipoles at low k to
                !keep the large-scale ISW smooth across the KG->EFA switch
                EV%lmaxnr=max(3,nint(min(8,nint(sqrt(scal)* 450 * EV%q))*CP%Accuracy%lAccuracyBoost))
            else
    ...
    use constants, only : c !AxiECAMB: for H0 in Mpc^-1 in the axion field ICs
    implicit none
    ...
    integer, parameter :: i_clxg=1,i_clxr=2,i_clxc=3, i_clxb=4, &
        ...
        i_dphi_ax=13, i_dphidot_ax=14 !AxiECAMB axion KG variables
    integer, parameter :: i_max = i_dphidot_ax
    real(dl) initv(6,1:i_max), initvec(1:i_max)
    real(dl) v1_bg, v2_bg, H0_mpc, adotoa_init !AxiECAMB
    ...
    if (State%HasAxion()) then
        !AxiECAMB adiabatic axion ICs (equations_ppf.f90:2577-2580): deep in RD with
        !m << H the field is frozen and the leading forced response to hL-dot is
        !delta-phi = m^2 (k z) a^2 phibar/(210 H^3), delta-phi-dot = m^2 (k z) a^2 phibar/(35 H^2),
        !with (k z)_adiabatic = chi*Kf(1)*(1-omtau/5)*k*x/2 analytically
        call CP%Axion%FieldValsAta(a, v1_bg, v2_bg)
        H0_mpc = CP%H0/(c/1.0d3)
        adotoa_init = 1.0_dl/(a*dtauda(State,a))
        initv(1,i_dphi_ax) = ((CP%Axion%m_ovH0*H0_mpc)**2.0d0)* &
            (chi*EV%Kf(1)*(1-omtau/5)*k*x/2)*a2*v1_bg/(210.0d0*(adotoa_init**3.0d0))
        initv(1,i_dphidot_ax) = (CP%Axion%m_ovH0**2.0d0)*H0_mpc &
            *(chi*EV%Kf(1)*(1-omtau/5)*k*x/2)*a2*v1_bg/(35.0d0*(adotoa_init**2.0d0))
    end if
    ...
    !AxiECAMB axion: per-k switch state and the delta-phi conditioning rescale
    !(equations_ppf.f90:2728-2737); renorm_c keeps the stored delta-phi O(1) over the
    !huge dynamic range at large m. Its value is physically irrelevant (pure rescale).
    if (State%HasAxion()) then
        EV%oscillation_started = .false.
        EV%metric_delta = 0._dl
        if (CP%Axion%has_switch) then
            EV%renorm_c = sqrt(CP%Axion%rhorefp_ovh2)*CP%H0*((k*CP%Axion%tau_osc)**2._dl)/CP%Axion%m_ovH0
            if (.not. (EV%renorm_c > 0._dl)) EV%renorm_c = 1._dl
        else
            EV%renorm_c = 1._dl
        end if
        y(EV%axion_ix) = InitVec(i_dphi_ax)/EV%renorm_c
        y(EV%axion_ix+1) = InitVec(i_dphidot_ax)
    end if
    ...
    use constants, only : barssc0, Compton_CT, line21_const, c !AxiECAMB: added c
    use MassiveNu
    use Recombination, only : CB1
    implicit none
    type(EvolutionVars) EV
    ...
    !AxiECAMB axion variables
    real(dl) v1_bg, v2_bg, dv1, dv2, drhoax_kg, grhoax_kg, clxax_kg, clxax_kg_dot
    real(dl) u_ax_kg, u_ax_kg_dot
    real(dl) grhoax_t, gpres_ax, w_ax, w_ax_p1, wcorr_coeff, csquared_ax, cad2
    real(dl) dorp, kamnorm, H0_mpc
    ...
    !AxiECAMB axion: background and perturbation variables in the current regime
    !(AxiECAMB equations_ppf.f90:3130-3174)
    grhoax_t=0; gpres_ax=0; clxax_kg=0; u_ax_kg=0
    dv1=0; dv2=0; v1_bg=0; v2_bg=0; w_ax=0; w_ax_p1=1
    if (State%HasAxion()) then
        associate (Ax => CP%Axion)
            H0_mpc = CP%H0/(c/1.0d3)
            if (.not. EV%oscillation_started) then
                !exact Klein-Gordon regime: delta-phi = ay(axion_ix)*renorm_c, delta-phi-dot = ay(axion_ix+1)
                dv1 = ay(EV%axion_ix)
                dv2 = ay(EV%axion_ix+1)
                call Ax%FieldValsAta(a, v1_bg, v2_bg)
                w_ax_p1 = (2.0_dl*(v2_bg**2.0_dl)/a2)/((v2_bg**2.0_dl)/a2 + (Ax%m_ovH0*v1_bg)**2.0_dl)
                w_ax = w_ax_p1 - 1.0_dl
                drhoax_kg = (v2_bg*dv2/a2 + (Ax%m_ovH0**2.0d0)*v1_bg*dv1*EV%renorm_c)*2.0d0
                grhoax_kg = (v2_bg)**2.0d0/a2 + (Ax%m_ovH0*v1_bg)**2.0d0
                if (v1_bg == v1_bg .and. v2_bg == v2_bg) then  !NaN guard
                    dorp = grhom_ovh2*grhoax_kg
                else
                    dorp = 0.0d0
                end if
                clxax_kg = drhoax_kg/grhoax_kg
                u_ax_kg = w_ax_p1*k*dv1*EV%renorm_c/(H0_mpc*v2_bg)
            else
                !effective-fluid regime past tau_osc
                wcorr_coeff = Ax%wcorr_coeff
                w_ax = ((wcorr_coeff/a2)**2.0d0)*Ax%wEFA_c
                w_ax_p1 = 1.0_dl + w_ax
                dorp = State%grhocrit*Ax%rhorefp_ovh2*((Ax%a_osc/a)**3.0d0)*dexp((wcorr_coeff**2.0d0)*3.0d0* &
                    Ax%wEFA_c*(1.0d0/(a2**2.0d0) - 1.0d0/(Ax%a_osc**4.0d0))/4.0d0)
                clxax_kg = ay(EV%axion_ix)
                u_ax_kg = ay(EV%axion_ix+1)
            end if
            grhoax_t = dorp*a2
    ...
    !AxiECAMB: axion density and pressure in the totals (grho_matter is kept clean so
    !Transfer_tot can apply the m/H0 >= 10 DM-like criterion separately)
    grho = grho_matter+grhor_t+grhog_t+grhov_t+grhoax_t
    ...
    !AxiECAMB: axion included here so the RSA reconstructions and the z constraint see it
    dgrho = dgrho_matter + grhoax_t*clxax_kg
    ...
    !AxiECAMB: axion heat flux added to 8*pi*a*a*SUM[(rho_i+p_i)*v_i] (no anisotropic stress)
    dgq=dgq + grhog_t*qg+grhor_t*qr + grhoax_t*u_ax_kg
    ...
    !AxiECAMB axion equations of motion (equations_ppf.f90:3384-3430)
    if (State%HasAxion()) then
        associate (Ax => CP%Axion)
            if (.not. EV%oscillation_started) then
                !exact KG in synchronous gauge, conformal time in Mpc:
                !delta-phi-dot' = -2 H delta-phi-dot - (k^2 + a^2 m^2) delta-phi - k z phibar-dot
                ayprime(EV%axion_ix) = dv2*H0_mpc/EV%renorm_c
                ayprime(EV%axion_ix+1) = -2*adotoa*dv2 - k2*dv1*EV%renorm_c/H0_mpc &
                    - a2*(Ax%m_ovH0**2.0_dl)*dv1*EV%renorm_c*H0_mpc - k*z*v2_bg
            else
                !effective fluid: w = wEFA_c (H/m)^2, scale-dependent sound speed
                cad2 = w_ax*((1._dl + (gpres_noDE + w_dark_energy_t*grhov_t)/grho)/w_ax_p1 + 1._dl)
                kamnorm = k2/(a2*((Ax%m_ovH0*H0_mpc)**2.0_dl))
                if (kamnorm < 1.e-14_dl) then
                    !Taylor branch to avoid catastrophic cancellation
                    csquared_ax = kamnorm/4.0_dl + 5.0_dl*(adotoa**2.0_dl)/(4.0_dl*(k2/kamnorm))
                else
                    csquared_ax = (sqrt(1.0_dl + kamnorm) - 1.0_dl)**2.0_dl/(kamnorm) &
                        + 5.0_dl*(adotoa**2.0_dl)/(4.0_dl*(k2/kamnorm))
                end if
                ...
                ayprime(EV%axion_ix) = clxax_kg_dot
                ayprime(EV%axion_ix+1) = u_ax_kg_dot
            end if
        end associate
    end if
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (associated(EV%OutputTransfer)) then
            ...
            if (State%HasAxion()) then
                if (.not. CP%Axion%is_de_like) then
                    !DM-like axion counts in the total matter (AxiECAMB outtransf:3051-3054,
                    !criterion m/H0 >= 10, consistent with the halofit treatment)
                    EV%OutputTransfer(Transfer_tot) = (dgrho_matter + grhoax_t*clxax_kg) &
                        /(grho_matter + grhoax_t)
                else
                    EV%OutputTransfer(Transfer_tot) =  dgrho_matter/grho_matter
                end if
                EV%OutputTransfer(Transfer_axion) = clxax_kg
            else
