    ...
    allocate(TAxionModel::P%Axion) !AxiECAMB: allocated by default with active=.false.
    ...
    if (Ini%Read_Logical('use_physical', .true.)) then
        ...
        !AxiECAMB axion parameters (m_ax, omaxh2 or omdah2/axfrac, ...)
        if (.not. allocated(P%Axion)) allocate(TAxionModel::P%Axion)
        call P%Axion%ReadParams(Ini)
        if (P%Axion%active .and. P%Axion%use_axfrac) then
            !omch2 is derived from omdah2 and axfrac in SetParams; allow it to be omitted
            P%omch2 = Ini%Read_Double('omch2', 0._dl)
        else
