...
Key facts:
- Public repo: `New_AxiECAMB/` is the local clone of `git@github.com:SBU-COSMOLIKE/NewTestingAxieCAMB.git` (branches `main` + `hmcode`). Its `README.rst` (~335 KB on `main`) is the generated developer documentation (the Phase-1 reports/Phase-3 design turned into docs); `README_AxiECAMB.md` is the short human summary; `.port_analysis/` holds the source diffs/reports/rst.
