...
1. `cosmo_dic` — plain dict (bypass `load_cosmology_input`; build it directly). Required keys
   ...
   h, z (SCALAR — one redshift per call), ns, As, k_piv (1/Mpc), M_min, M_max (defaults 1e8,
   1e17 Msun/h; note input_file.txt writes exponents 7/18 — check which the notebook uses),
...
2. `power_spec_dic` — from transfer functions via `func_power_spec_dic`
   ...
   verify numerically once (V1). NOTE: two PS dicts appear in the pipeline (one for sigma
   computations) — mirror the example notebook exactly on this point.
...
5. `P_NL = func_full_halo_model_ax(M, power_spec_dic, cosmo_dic, hmcode_dic, axion_dic,
   ...
   M is the halo-mass integration grid (Msun/h).
...
- numba @njit in halo_bias.py and variance.py (Dome update, Lague) — first call pays JIT
  compilation; ensure numba is in the Cocoa environment.
